...
import { PrismaClient, UserRole, WorkActaStatus, CostActaStatus } from '@prisma/client'; // Asegúrate de importar WorkActaStatus
...
app.post('/api/auth/register', async (req, res) => { res.status(501).json({ message: 'Not Implemented' }); });
app.post('/api/auth/login', async (req, res) => { res.status(501).json({ message: 'Not Implemented' }); });
app.get('/api/users', async (req, res) => {
    try {
        const users = await prisma.user.findMany({ select: { id: true, fullName: true, email: true, projectRole: true, avatarUrl: true, appRole: true, status: true } });
        res.json(users);
    } catch (error) {
...
app.get('/api/communications', async (req, res) => {
  try {
    ...
    const formattedComms = communications.map(comm => ({
        ...comm,
        senderDetails: { entity: comm.senderEntity, personName: comm.senderName, personTitle: comm.senderTitle },
        recipientDetails: { entity: comm.recipientEntity, personName: comm.recipientName, personTitle: comm.recipientTitle },
        statusHistory: [],
        attachments: []
    }));
...
app.post('/api/communications', async (req, res) => {
    try {
        const {
            radicado, subject, description, senderDetails, recipientDetails,
            signerName, sentDate, dueDate, deliveryMethod, notes, parentId, uploaderId
        } = req.body;
        const prismaDeliveryMethod = deliveryMethodMap[deliveryMethod] || 'SYSTEM';
        const newComm = await prisma.communication.create({
            data: {
                radicado, subject, description,
                senderEntity: senderDetails.entity, senderName: senderDetails.personName, senderTitle: senderDetails.personTitle,
                recipientEntity: recipientDetails.entity, recipientName: recipientDetails.personName, recipientTitle: recipientDetails.personTitle,
                signerName, sentDate: new Date(sentDate), dueDate: dueDate ? new Date(dueDate) : null,
                deliveryMethod: prismaDeliveryMethod, notes,
        ...
        const formattedComm = {
            ...newComm,
            senderDetails: { entity: newComm.senderEntity, personName: newComm.senderName, personTitle: newComm.senderTitle },
            recipientDetails: { entity: newComm.recipientEntity, personName: newComm.recipientName, personTitle: newComm.recipientTitle },
...
app.get('/api/drawings', async (req, res) => {
  try {
    const drawings = await prisma.drawing.findMany({
      ...
      include: {
        versions: { orderBy: { versionNumber: 'desc' }, include: { uploader: true } },
    ...
    const formattedDrawings = drawings.map(d => ({
        ...d,
        discipline: Object.keys(drawingDisciplineMap).find(key => drawingDisciplineMap[key] === d.discipline) || d.discipline
    }));
...
app.post('/api/drawings', async (req, res) => {
  try {
    ...
    if (!code || !title || !discipline || !version) {
      return res.status(400).json({ error: 'Faltan datos para crear el plano.' });
    }
    const newDrawing = await prisma.drawing.create({
      data: {
        code, title, discipline: prismaDiscipline,
        versions: {
          create: [{
            versionNumber: 1, fileName: version.fileName, url: version.url, size: version.size,
            uploader: { connect: { id: version.uploaderId } },
          }],
        },
      },
      include: { versions: { include: { uploader: true } }, comments: { include: { author: true } } } // Incluir comments
    });
     // Formatear disciplina en la respuesta
     const formattedDrawing = {
        ...newDrawing,
        discipline: Object.keys(drawingDisciplineMap).find(key => drawingDisciplineMap[key] === newDrawing.discipline) || newDrawing.discipline
    };
    res.status(201).json(formattedDrawing);
  } catch (error) {
    console.error("Error al crear el plano:", error);
    if ((error as any).code === 'P2002') {
         return res.status(409).json({ error: 'Ya existe un plano con este código.' });
    }
...
app.post('/api/drawings/:id/versions', async (req, res) => {
  try {
    ...
    if (!version) {
      return res.status(400).json({ error: 'Faltan los datos de la nueva versión.' });
    }
    ...
    const updatedDrawing = await prisma.drawing.update({
      ...
      data: {
        ...
        versions: {
          create: {
            versionNumber: latestVersionNumber + 1, fileName: version.fileName, url: version.url, size: version.size,
            uploader: { connect: { id: version.uploaderId } },
          }
        }
      },
      include: { versions: { orderBy: { versionNumber: 'desc' }, include: { uploader: true } }, comments: { include: { author: true } } } // Incluir comments
    });
    // Formatear disciplina en la respuesta
    const formattedDrawing = {
        ...updatedDrawing,
        discipline: Object.keys(drawingDisciplineMap).find(key => drawingDisciplineMap[key] === updatedDrawing.discipline) || updatedDrawing.discipline
    };
    res.status(201).json(formattedDrawing);
  } catch (error) {
...
app.post('/api/drawings/:id/comments', async (req, res) => {
  try {
    ...
    if (!content || !authorId) {
      return res.status(400).json({ error: 'El contenido y el autor son obligatorios.' });
    }
...
app.get('/api/actas', async (req, res) => {
  try {
    ...
    const formattedActas = actas.map(acta => ({
        ...acta,
        area: Object.keys(actaAreaMap).find(key => actaAreaMap[key] === acta.area) || acta.area,
        status: Object.keys(actaStatusMap).find(key => actaStatusMap[key] === acta.status) || acta.status,
    }));
...
app.post('/api/actas', async (req, res) => {
  try {
    const { number, title, date, area, status, summary, commitments = [], attachments = [], requiredSignatories = [] } = req.body;
      ...
      data: {
        ...
        commitments: {
          create: commitments.map((c: any) => ({
            description: c.description, dueDate: new Date(c.dueDate), status: 'PENDING',
            responsible: { connect: { id: c.responsible.id } }
          }))
        },
        attachments: {
          create: attachments.map((att: any) => ({
            fileName: att.fileName, url: att.url, size: att.size, type: att.type,
          }))
        },
        requiredSignatoriesJson: JSON.stringify(requiredSignatories.map((u: any) => u.id))
      },
      include: {
        commitments: { include: { responsible: true } }, attachments: true, signatures: { include: { signer: true } }
      }
    });
    // Formatear enums en la respuesta
    const formattedActa = {
        ...newActa,
        area: Object.keys(actaAreaMap).find(key => actaAreaMap[key] === newActa.area) || newActa.area,
        status: Object.keys(actaStatusMap).find(key => actaStatusMap[key] === newActa.status) || newActa.status,
    };
...
app.get('/api/log-entries', async (req, res) => {
  try {
    const entries = await prisma.logEntry.findMany({
      orderBy: { createdAt: 'desc' },
      include: {
        author: true, attachments: true,
        comments: { include: { author: true }, orderBy: { timestamp: 'asc' },},
        signatures: { include: { signer: true }}, assignees: true,
      }
    });
    // Formatear enums antes de enviar
    const formattedEntries = entries.map(entry => ({
        ...entry,
        type: Object.keys(entryTypeMap).find(key => entryTypeMap[key] === entry.type) || entry.type,
        status: Object.keys(entryStatusMap).find(key => entryStatusMap[key] === entry.status) || entry.status,
    }));
...
app.post('/api/log-entries', async (req, res) => {
  try {
    const { title, description, type, subject, location, activityStartDate, activityEndDate, isConfidential, status, authorId, projectId, assignees = [] } = req.body;
    if (!title || !authorId || !projectId) {
      return res.status(400).json({ error: 'Título, autor y proyecto son obligatorios.' });
    }
    ...
    const newEntry = await prisma.logEntry.create({
      data: {
        title, description, type: prismaType, subject, location,
        activityStartDate: new Date(activityStartDate), activityEndDate: new Date(activityEndDate),
        isConfidential, status: prismaStatus,
        author: { connect: { id: authorId } },
        project: { connect: { id: projectId } },
        assignees: { connect: assignees.map((user: { id: string }) => ({ id: user.id })) }
      },
      include: { author: true, attachments: true, comments: true, signatures: true, assignees: true, }
    });
     // Formatear enums en la respuesta
     const formattedEntry = {
        ...newEntry,
        type: Object.keys(entryTypeMap).find(key => entryTypeMap[key] === newEntry.type) || newEntry.type,
        status: Object.keys(entryStatusMap).find(key => entryStatusMap[key] === newEntry.status) || newEntry.status,
    };
...
app.put('/api/log-entries/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const { title, description, type, subject, location, activityStartDate, activityEndDate, isConfidential, status, assignees = [] } = req.body;
    const prismaType = entryTypeMap[type] || 'GENERAL';
    const prismaStatus = entryStatusMap[status] || 'DRAFT';
    const updatedEntry = await prisma.logEntry.update({
      ...
      data: {
        ...
        activityStartDate: new Date(activityStartDate), activityEndDate: new Date(activityEndDate),
        isConfidential, status: prismaStatus,
        assignees: { set: assignees.map((user: { id: string }) => ({ id: user.id })) }
      },
      include: {
        author: true, attachments: true,
        comments: { include: { author: true }, orderBy: { timestamp: 'asc' }},
     ...
     const formattedEntry = {
        ...updatedEntry,
        type: Object.keys(entryTypeMap).find(key => entryTypeMap[key] === updatedEntry.type) || updatedEntry.type,
        status: Object.keys(entryStatusMap).find(key => entryStatusMap[key] === updatedEntry.status) || updatedEntry.status,
    };
...
app.get('/api/contract-items', async (req, res) => {
  try {
    const items = await prisma.contractItem.findMany({ orderBy: { itemCode: 'asc' } });
    res.json(items);
  } catch (error) {
    console.error("Error al obtener los ítems del contrato:", error);
    res.status(500).json({ error: 'No se pudieron obtener los ítems del contrato.' });
  }
});
...
app.get('/api/work-actas', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'No se pudieron obtener las actas de avance.' });
  }
});
...
app.post('/api/work-actas', async (req, res) => {
  try {
    ...
    if (!number || !period || !date || !items || items.length === 0) {
      return res.status(400).json({ error: 'Faltan datos para crear el acta de avance.' });
    }
    ...
    const newActa = await prisma.workActa.create({
      data: {
        ...
        items: {
          create: items.map((item: { contractItemId: string, quantity: number }) => ({
    ...
    if ((error as any).code === 'P2002') {
         return res.status(409).json({ error: 'Ya existe un acta de avance con este número.' });
    }
...
app.put('/api/work-actas/:id', async (req, res) => {
  try {
    ...
    if (!prismaStatus || !Object.values(WorkActaStatus).includes(prismaStatus)) {
...
app.get('/api/cost-actas', async (req, res) => {
  try {
    const actas = await prisma.costActa.findMany({
      ...
      include: {
        observations: { // Incluimos observaciones
          include: { author: true },
    ...
    const formattedActas = actas.map(acta => ({
        ...acta,
        // Aquí no necesitamos mapa, ya que los valores del enum son los que queremos mostrar
        // Si quisiéramos textos más largos, crearíamos un mapa en enum-maps.ts
    }));
    res.json(formattedActas);
  } catch (error) {
    console.error("Error al obtener las actas de costo:", error);
    res.status(500).json({ error: 'No se pudieron obtener las actas de costo.' });
  }
});
...
app.post('/api/cost-actas', async (req, res) => {
  try {
    ...
    const {
      number, period, submissionDate, billedAmount, totalContractValue,
      relatedProgress, attachments = [] // attachments es ahora un array de objetos { id: string }
    } = req.body;

    if (!number || !period || !submissionDate || !billedAmount || !totalContractValue) {
      return res.status(400).json({ error: 'Faltan datos obligatorios para crear el acta de costo.' });
    }
    ...
    const newCostActa = await prisma.costActa.create({
      ...
      include: { // Devolvemos el acta creada completa
    ...
    if ((error as any).code === 'P2002') { // Error de código único duplicado
         return res.status(409).json({ error: 'Ya existe un acta de costo con este número.' });
    }
