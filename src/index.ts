...
app.post(
  ...
  async (req: AuthRequest, res) => {
    ...
    if (!query) {
      return res
        .status(400)
        .json({ error: "No se proporcionó una consulta (query)." });
    }
    if (!userId) {
      return res.status(401).json({ error: "Usuario no autenticado." });
    }
    ...
    try {
      ...
      if (project) {
        ...
        if (currentEndDate) {
          currentEndDate.setDate(
            currentEndDate.getDate() + totalExtensionsDays
          );
        }
      ...
      if (contractItems.length) {
        const itemsWithProgress = contractItems.map((item) => {
          ...
          if (latestEntry?.workActa) {
            ...
            lastActaSummary = ` Último reporte: acta ${
              acta.number
            } (${formatDate(acta.date)}) en estado ${actaStatus}.`;
          }
        ...
        if (topItems.length) {
          const lines = topItems.map(
            (item) =>
              `• ${item.itemCode} - ${
                item.description
              }: Contratado ${formatNumber(item.contractQuantity, 2)} ${
                item.unit
              }, Ejecutado ${formatNumber(item.executedQuantity, 2)} ${
                item.unit
              } (avance ${formatPercentage(item.percentage, 1)}).${
      ...
      if (workActas.length) {
        const workActaLines = workActas.map((acta) => {
          ...
          const status = workActaStatusReverseMap[acta.status] || acta.status;
      ...
      if (communications.length) {
        const communicationsSummary = communications
          .map((comm) => {
            ...
            return `• Radicado ${comm.radicado}: "${
              comm.subject
            }" - De: ${sender} - Para: ${recipient} - Estado: ${status} - Fecha: ${formatDate(
              comm.sentDate
            )}`;
          })
          .join("\n");
      ...
      if (actas.length) {
        const actasSummary = actas
          .map((acta) => {
            ...
            return `• ${acta.number}: "${
              acta.title
            }" - Área: ${area} - Estado: ${status} - Compromisos: ${commitmentsCount} - Fecha: ${formatDate(
              acta.date
            )}`;
          })
          .join("\n");

        contextoSecciones.push(`Actas de comité recientes:\n${actasSummary}`);
      }
      ...
      if (costActas.length) {
        const costActasSummary = costActas
          .map((acta) => {
            ...
            return `• ${acta.number}: Período ${
              acta.period
            } - Valor: ${formatCurrency(
              acta.billedAmount
            )} - Estado: ${status} - Fecha: ${formatDate(acta.submissionDate)}`;
          })
          .join("\n");
      ...
      if (reports.length) {
        const reportsSummary = reports
          .map((report) => {
            ...
            return `• ${report.type} ${
              report.number
            }: ${scope} - Estado: ${status} - Autor: ${
              report.author?.fullName
            } - Fecha: ${formatDate(report.submissionDate)}`;
          })
          .join("\n");

        contextoSecciones.push(`Informes recientes:\n${reportsSummary}`);
      }
      ...
      if (drawings.length) {
        ...
        contextoSecciones.push(`Planos del proyecto:\n${drawingsSummary}`);
      }
      ...
      if (pendingCommitments.length) {
        const commitmentsSummary = pendingCommitments
          .map((commitment) => {
            ...
            } - Responsable: ${responsible} (${role}) - Vence: ${formatDate(
              commitment.dueDate
            )}`;
          })
          .join("\n");
      ...
      if (recentLogEntries.length) {
        const logEntriesSummary = recentLogEntries
          .map((entry) => {
            ...
            return `• "${
              entry.title
            }" - Autor: ${author} - Tipo: ${type} - Estado: ${status} - Asignados: ${assignees} - Fecha: ${formatDate(
              entry.createdAt
            )}`;
          })
          .join("\n");
      ...
      if (error.response) {
        ...
        if (error.response.status === 429) {
          return res
            ...
            .json({
              error:
                "Límite de cuota de OpenAI excedido. Revisa tu facturación.",
            });
        }
      }
      res
        .status(500)
        .json({ error: "Error al procesar la respuesta del chatbot." });
    }
  }
);
...
app.post("/api/upload", async (req, res) => {
  try {
    ...
    if ((error as any).code === "P2002") {
      return res
        .status(409)
        .json({ error: "Ya existe un archivo con este nombre." });
    }
...
app.post("/api/auth/reset-password/:token", async (req, res) => {
  ...
  try {
    ...
    if (resetToken.expiresAt < new Date()) {
      return res.status(400).json({
        error:
          "El token ha expirado. Solicita un nuevo enlace de restablecimiento.",
      });
    }
...
app.put("/api/auth/profile", authMiddleware, async (req: AuthRequest, res) => {
  ...
  console.log(
    "Actualización de perfil solicitada por usuario:",
    userId,
    req.body
  );
  ...
  try {
    ...
    const updatedUser = await prisma.user.update({
      ...
      data: updateData,
      select: {
        // Devuelve solo los datos públicos
        id: true,
        fullName: true,
        email: true,
        projectRole: true,
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      if (projectRole) {
        ...
        if (!resolved) {
          return res.status(400).json({ error: "Rol de proyecto inválido." });
        }
...
app.get(
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: "Error al obtener modificaciones contractuales." });
    }
  }
);
...
app.get("/api/communications/:id", async (req, res) => {
  try {
    ...
    formattedComm.attachments = (communication.attachments || []).map(
      buildAttachmentResponse
    );
    ...
    res
      .status(500)
      .json({ error: "No se pudo obtener la comunicación solicitada." });
  }
});
...
app.post("/api/communications", async (req, res) => {
  try {
    ...
    formattedComm.attachments = (newComm.attachments || []).map(
      buildAttachmentResponse
    );
  ...
  uploadMiddleware(req, res, async (err) => {
    ...
    try {
      ...
      if (req.body.requiredSignatories) {
        try {
          formData.requiredSignatories = JSON.parse(
            req.body.requiredSignatories
          );
        } catch (e) {
      ...
      formData.interventoriaPersonnel = parseJsonField(
      ...
      const isConfidentialValue = parseBoolean(
        (formData as any).isConfidential
      );
      ...
      const requiredSignerIds = extractUserIds(
        (formData as any).requiredSignatories
      );
      ...
      const rawEntryDate =
        (formData as any).entryDate || formData.activityStartDate;
      const parsedEntryDate = new Date(rawEntryDate);

      if (isNaN(parsedEntryDate.getTime())) {
        return res
          .status(400)
          .json({ error: "La fecha de la entrada de bitácora no es válida." });
      }
      ...
      const additionalObservations =
        (formData as any).additionalObservations ?? "";

      // Validar attachments
      if (!Array.isArray(attachments)) {
        return res
          .status(400)
          .json({ error: "El formato de los adjuntos no es válido." });
      }
        ...
        data: {
          ...
          locationDetails:
            typeof (formData as any).locationDetails === "string"
              ...
              : "",
          weatherReport:
            normalizedWeatherReport as unknown as Prisma.InputJsonValue,
          contractorPersonnel:
            normalizedContractorPersonnel as unknown as Prisma.InputJsonValue,
          interventoriaPersonnel:
            normalizedInterventoriaPersonnel as unknown as Prisma.InputJsonValue,
          equipmentResources:
            normalizedEquipmentResources as unknown as Prisma.InputJsonValue,
          executedActivities:
            normalizedExecutedActivities as unknown as Prisma.InputJsonValue,
          executedQuantities:
            normalizedExecutedQuantities as unknown as Prisma.InputJsonValue,
          scheduledActivities:
            normalizedScheduledActivities as unknown as Prisma.InputJsonValue,
          qualityControls:
            normalizedQualityControls as unknown as Prisma.InputJsonValue,
          materialsReceived:
            normalizedMaterialsReceived as unknown as Prisma.InputJsonValue,
          safetyNotes:
            normalizedSafetyNotes as unknown as Prisma.InputJsonValue,
          projectIssues:
            normalizedProjectIssues as unknown as Prisma.InputJsonValue,
      ...
      await recordLogEntryChanges(
        newEntry.id,
        req.user?.userId || formData.authorId,
        creationChanges
      );
      ...
      if (error.code === "P2002") {
        ...
        if (
          ...
          return res.status(409).json({
            error:
              "Ya existe una entrada de bitácora registrada para este día en el proyecto.",
          });
        }
        return res.status(409).json({
      ...
      if (error.code === "P2025") {
        return res.status(404).json({
          error:
            "No se encontró alguno de los recursos referenciados (autor, proyecto, asignados o adjuntos).",
        });
      }
      ...
      if (error.message === "Invalid JSON") {
        return res.status(400).json({
          ...
          details:
            "Los datos enviados no tienen un formato válido. Asegúrate de enviar los datos correctamente.",
        });
      }
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      if (requiredSignatoriesPayload !== undefined) {
        ...
        const existingSignerIds = updatedSignatureTasks.map(
          (task: any) => task.signerId
        );

        const toAdd = newSignerIds.filter(
          (id: string) => !existingSignerIds.includes(id)
        );
        const toRemove = existingSignerIds.filter(
          (id: string) => !newSignerIds.includes(id)
        );
        ...
        const pendingCount = updatedSignatureTasks.filter(
          (task: any) => task.status !== "SIGNED"
        ).length;
      ...
      if (isConfidential !== undefined)
        dataToUpdate.isConfidential = isConfidential;
      if (status !== undefined) dataToUpdate.status = prismaStatus;
      if (activitiesPerformed !== undefined)
        dataToUpdate.activitiesPerformed = String(activitiesPerformed ?? "");
      if (materialsUsed !== undefined)
        dataToUpdate.materialsUsed = String(materialsUsed ?? "");
      if (workforce !== undefined)
        dataToUpdate.workforce = String(workforce ?? "");
      if (weatherConditions !== undefined)
        dataToUpdate.weatherConditions = String(weatherConditions ?? "");
      if (additionalObservations !== undefined)
        dataToUpdate.additionalObservations = String(
          additionalObservations ?? ""
        );
      if (scheduleDay !== undefined) {
        dataToUpdate.scheduleDay =
          typeof scheduleDay === "string" ? scheduleDay.trim() : "";
      }
      if (locationDetails !== undefined) {
        dataToUpdate.locationDetails =
          typeof locationDetails === "string" ? locationDetails.trim() : "";
      }
      ...
      if (contractorPersonnel !== undefined) {
        dataToUpdate.contractorPersonnel =
          normalizePersonnelEntries(contractorPersonnel);
      }
      if (interventoriaPersonnel !== undefined) {
        dataToUpdate.interventoriaPersonnel = normalizePersonnelEntries(
          interventoriaPersonnel
        );
      }
      if (equipmentResources !== undefined) {
        dataToUpdate.equipmentResources =
          normalizeEquipmentEntries(equipmentResources);
      }
      if (executedActivities !== undefined) {
        dataToUpdate.executedActivities =
          normalizeListItems(executedActivities);
      }
      if (executedQuantities !== undefined) {
        dataToUpdate.executedQuantities =
          normalizeListItems(executedQuantities);
      }
      if (scheduledActivities !== undefined) {
        dataToUpdate.scheduledActivities =
          normalizeListItems(scheduledActivities);
      }
      ...
      if (contractorObservations !== undefined) {
        dataToUpdate.contractorObservations =
          typeof contractorObservations === "string"
            ? contractorObservations.trim()
            : "";
      }
      if (interventoriaObservations !== undefined) {
        dataToUpdate.interventoriaObservations =
          typeof interventoriaObservations === "string"
      ...
      if (
        ...
        const baseDate =
          entryDate || activityStartDate || existingEntry.entryDate;
        const parsed = new Date(baseDate);
        if (isNaN(parsed.getTime())) {
          return res
            .status(400)
            .json({ error: "La fecha de la entrada no es válida." });
        }
      ...
      if (title !== undefined && title !== existingEntry.title) {
        changes.push({
          fieldName: "Título",
          oldValue: existingEntry.title,
          newValue: updatedEntry.title,
        });
      }
      if (
        ...
        changes.push({
          ...
          oldValue: existingEntry.description,
          newValue: updatedEntry.description,
        });
      }
      if (type !== undefined && prismaType !== existingEntry.type) {
        changes.push({
          fieldName: "Tipo",
          oldValue: entryTypeReverseMap[existingEntry.type],
          newValue: entryTypeReverseMap[updatedEntry.type],
        });
      }
      if (subject !== undefined && subject !== existingEntry.subject) {
        changes.push({
          fieldName: "Asunto",
          oldValue: existingEntry.subject,
          newValue: updatedEntry.subject,
        });
      }
      if (location !== undefined && location !== existingEntry.location) {
        changes.push({
          fieldName: "Ubicación",
          oldValue: existingEntry.location,
          newValue: updatedEntry.location,
        });
      }
      if (dataToUpdate.entryDate) {
        ...
        if (oldStart !== newStart) {
          changes.push({
            fieldName: "Fecha Inicio Actividad",
            oldValue: oldStart,
            newValue: newStart,
          });
        }
      ...
      if (
        ...
        isConfidential !== existingEntry.isConfidential
      ) {
        changes.push({
      ...
      if (
        ...
        activitiesPerformed !== existingEntry.activitiesPerformed
      ) {
        changes.push({
      ...
      if (
        ...
        materialsUsed !== existingEntry.materialsUsed
      ) {
        changes.push({
      ...
      if (
        ...
        weatherConditions !== existingEntry.weatherConditions
      ) {
        changes.push({
      ...
      if (
        ...
        additionalObservations !== existingEntry.additionalObservations
      ) {
        changes.push({
      ...
      const previousAttachmentIds = new Map(
        (existingEntry.attachments || []).map((att) => [att.id, att])
      );
      const newAttachmentIds = new Map(
        (updatedEntry.attachments || []).map((att) => [att.id, att])
      );
      ...
      const previousAssignees = new Map(
        (existingEntry.assignees || []).map((user) => [user.id, user])
      );
      const newAssignees = new Map(
        (updatedEntry.assignees || []).map((user) => [user.id, user])
      );
...
app.delete(
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      // Verificar permisos
      if (
        existingEntry.authorId !== req.user?.userId &&
        req.user?.appRole !== "admin"
      ) {
        console.log("Usuario no autorizado para eliminar esta anotación");
        return res
          .status(403)
          .json({ error: "No tiene permisos para eliminar esta anotación." });
      }
      ...
      console.log(
        `DELETE /api/log-entries/:id - Anotación eliminada con ID: ${id}`
      );
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      if (!content || !authorId) {
        return res
          .status(400)
          .json({ error: "Contenido y autor son obligatorios." });
      }
...
app.post(
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      const myTask = entryWithTasks.signatureTasks.find(
        (t) => t.signerId === signerId
      );
      if (!myTask) {
        return res
          .status(403)
      ...
      if (userSignature) {
        try {
          ...
          if (!basePdf) {
            console.log(
              "No hay PDF existente, generando uno automáticamente..."
            );
            try {
              const baseUrl =
                process.env.SERVER_PUBLIC_URL || `http://localhost:4001`;
          ...
          console.warn(
            "No se pudo aplicar la firma manuscrita automáticamente:",
            signatureError
          );
        }
      }
      ...
      if (
        ...
        const uniqueSignerIds = Array.from(
          new Set(updatedTasks.map((t) => t.signerId))
        );
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      if (
        ...
        error.message === "Anotación no encontrada."
      ) {
        return res.status(404).json({ error: error.message });
      }
      res
        .status(500)
        .json({ error: "No se pudo generar el PDF de la anotación." });
    }
  }
);
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      if (
        ...
        return res
          .status(400)
          .json({ error: "Estado inválido proporcionado." });
      }
      ...
      res
        .status(500)
        .json({ error: "No se pudo actualizar el acta de avance." });
    }
  }
);
...
app.get("/api/reports/:id", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "No se pudo obtener el informe solicitado." });
  }
});
...
app.post("/api/reports", async (req, res) => {
  try {
    ...
    let previousReportConnect: { connect: { id: string } } | undefined =
      undefined;
    ...
    try {
      ...
      if (!Array.isArray(incomingTasks)) {
        return res
          .status(400)
          .json({
            error: "Formato inválido. Envía tareas normalizadas o XML válido.",
          });
      }
      ...
      const sanitizedTasks = incomingTasks.map((task: any, index: number) => {
        ...
        if (name.length > MAX_NAME_LENGTH) {
          ...
          console.warn(
            `Truncating task name "${name}" to ${MAX_NAME_LENGTH} characters during import.`
          );
        }
        ...
        if (Number.isNaN(parsedStart.getTime())) {
          throw new Error(
            `La tarea "${safeName}" no tiene una fecha de inicio válida.`
          );
        }
        ...
        if (Number.isNaN(parsedEnd.getTime())) {
          throw new Error(
            `La tarea "${safeName}" no tiene una fecha de fin válida.`
          );
        }
...
app.post(
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      if (
        error instanceof Error &&
        error.message === "Informe no encontrado."
      ) {
        return res.status(404).json({ error: error.message });
      }
